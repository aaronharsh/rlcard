...
class TestSimpleCribbageEnv(unittest.TestCase):

    def test_init_game_and_extract_state(self):
        env = rlcard.make('simple-cribbage')
        state, _ = env.init_game()
        self.assertEqual(state['obs'].size, 32)
    ...
    def test_step_back(self):
        env = rlcard.make('simple-cribbage', config={'allow_step_back':True})
        ...
        env = rlcard.make('simple-cribbage', config={'allow_step_back':False})
        state, player_id = env.init_game()
        action = np.random.choice(state['legal_actions'])
        env.step(action)
        # env.step_back()
    ...
    def test_single_agent_mode(self):
        env = rlcard.make('simple-cribbage', config={'single_agent_mode':True})
        state = env.reset()
